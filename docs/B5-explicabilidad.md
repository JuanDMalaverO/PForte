...
- `scripts/evaluar-grabacion.mjs` — pasa una grabación real de la escala por los tres motores y compara ataque por ataque con la nota esperada; calibra las huellas con esa misma grabación en el medio. `scripts/evaluar-acordes.mjs` — calibra con la escala y evalúa la grabación de los 20 acordes con los tres motores. Son la forma de ajustar con datos y no a ciegas: cada cambio al motor se mide contra el piano real de Juan en 5 minutos.
...
3. "Grabar (3 s)": el clip pasa por `analizarClip` (compuerta: ¿hay señal 10 dB sobre el piso?), luego `DetectorHuellas.analizarClip`: ataques (solo los que traen subida de energía) → 12 frames tras el ataque → **selección dispersa en dos etapas**: primero se prueban las notas esperadas, de a una, siempre la que mejor explica lo que falta, mientras cada una reduzca el residuo al menos 1 %; después las vecinas (semitono y octava), que solo entran si explican al menos 5 %. Cada nota pasa además el chequeo de **coherencia de la fundamental** (si la huella predice energía en su fundamental y el espectro no la tiene, es un fantasma que solo explica armónicos ajenos). Se compara con lo esperado.
4. Bloque 4 con lista esperada: la grabación se corta por ataques, una primera pasada mira qué hay en cada ataque, un **alineamiento por contenido** (`metricas.ts`, programación dinámica) empareja ataques con acordes tolerando golpes de más o de menos, y la segunda pasada evalúa cada acorde como en el punto 3.
...
1. **Las coordenadas de OSMD (`partitura.ts`).** Sé que 1 unidad = 10 px y que `beginInstructionsWidth` es el ancho de clave + armadura + compás. Lo que no entendía era `Size.height` de un compás: **depende de las notas que tenga**, así que cuando cada tapa usaba la suya, la fila quedaba dentada. Ahora el alto sale de la caja de los pentagramas (`ParentStaffLine`), que es igual en todo el renglón, más un margen de 2.5 unidades para plicas, líneas adicionales y el número de compás; y el ancho se estira 0.6 unidades a la derecha porque la caja de un compás termina antes de su barra y si no queda un hilo de pentagrama entre tapa y tapa. Los dos márgenes siguen siendo a ojo. *Qué queda por entender:* qué incluye exactamente la caja del sistema (`ParentMusicSystem`), que sobra bastante por abajo.
...
3b. **Por qué calibrar con el celular empeoró los acordes.** Lo entiendo a medias: el micrófono del celular no capta bien por debajo de ~150 Hz, así que la huella "aprendida" de Do3 casi no tiene fundamental y se parece a Do4+Sol4. Lo que no sé es cuánto de eso es el micrófono y cuánto es el piano o la sala; haría falta repetir la grabación con otro micrófono para separarlo.
